...
async def criar_colaborador(data: ColaboradorCreate, db: AsyncSession = Depends(get_db)):
...
async def listar_colaboradores(skip: int = 0, limit: int = 50, db: AsyncSession = Depends(get_db)):
    stmt = select(Colaborador).offset(skip).limit(limit).order_by(Colaborador.nome)
    result = await db.execute(stmt)
    return result.scalars().all()


@router.get("/{colaborador_id}", response_model=ColaboradorFullResponse)
async def buscar_colaborador(colaborador_id: UUID, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Colaborador)
        .options(
...
async def atualizar_colaborador(
    colaborador_id: UUID, data: ColaboradorUpdate, db: AsyncSession = Depends(get_db)
):
...
async def deletar_colaborador(colaborador_id: UUID, db: AsyncSession = Depends(get_db)):
    stmt = select(Colaborador).where(Colaborador.id == colaborador_id)
    result = await db.execute(stmt)
    colaborador = result.scalar_one_or_none()
    if not colaborador:
...
async def registrar_medicao(
    colaborador_id: UUID, data: MedicaoSchema, db: AsyncSession = Depends(get_db)
):
...
async def listar_medicoes(colaborador_id: UUID, limit: int = 20, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Medicao)
...
async def registrar_condicao(
    colaborador_id: UUID, data: CondicaoSaudeSchema, db: AsyncSession = Depends(get_db)
):
...
async def registrar_preferencia(
    colaborador_id: UUID, data: PreferenciaSchema, db: AsyncSession = Depends(get_db)
):
